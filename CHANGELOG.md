...
- **PRD-029 P1.R7 — Generated types regenerated.** `lib/types/database.ts` reflects `workspace_notifications` after the migration applied; commit pairs the SQL with the generated TypeScript so the repository's `mapRow` is type-safe.
...
- **Increment 1.5 — End-of-part audit.** SRP confirmed (each file does one thing); DRY confirmed (RLS predicate appears twice across two policies because RLS doesn't let us factor — acceptable; otherwise no duplication); logging convention met (`[notification-service]` and `[supabase-notification-repo]` prefixes; entry/exit/error on every method); zero `any` leaks into public types (only the standard ESLint-suppressed `any` in `mapRow` matching the existing codebase pattern); zero dead code; named exports only; kebab-case files; PascalCase types; camelCase + Schema suffix on Zod schemas. ARCHITECTURE.md updated: tables list, new `workspace_notifications` schema subsection, file-map entries for the five new files, status line. CHANGELOG.md gets this entry. `npx tsc --noEmit` passes clean.
