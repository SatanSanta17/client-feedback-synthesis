...
2. **All in-app URL updates use raw `window.history` API.** Sidebar items become `<button onClick>` (NOT `<Link>`). "New chat" calls `history.pushState`, not `router.push`. First-send URL update also uses `history.pushState` so the natural browser history chain is preserved — back from `/chat/<id>` returns to `/chat` (the empty fresh-chat entry) before exiting the chat surface. None of these trigger Next.js page navigation, so the chat shell stays mounted across all in-app moves — sidebar clicks, new chat, first-send URL sync, browser back/forward.
