...
- **Public routes.** `/`, `/login`, `/signup`, `/forgot-password`, `/auth/callback`, `/invite/*`, and `/api/invite/*` are middleware-excluded. `/api/invite/*` is excluded because invited signups create the auth user (no session yet) and the accept handler self-gates on the token. `/reset-password` is authenticated (the recovery link establishes the session first). Keep this list in sync in `middleware.ts` when adding new auth pages.
