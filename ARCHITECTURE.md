...
│   │   ├── use-video-attachment.ts # PRD-032 Part 1 — per-attachment state machine: probe → extract → upload → transcribe. Exposes `state: VideoUploadState` and `cancel()`. AbortController propagated through every async stage; abort-checked at every await boundary. Latest-callback refs for `onCompleted`/`onAutoPersisted`/`onError` keep the effect keyed on `[file]` only (otherwise parent re-renders with a new opts object would abort in-flight extractions). **PRD-032 Part 2:** options gain `sessionId` and `onAutoPersisted`; the hook plumbs `sessionId` through to `uploadAudioForTranscription` and branches on `result.attachment` presence (present → `onAutoPersisted(SessionAttachment)`, absent → `onCompleted(VideoTranscriptAttachment)`). Activates the `EMPTY_TRANSCRIPT` code reserved in Part 1's enum via a verbatim-message regex match. **PRD-032 Part 4 (P4.R1, P4.R2):** `mapToVideoUploadError(err)` heuristic refined — explicit `instanceof RangeError` + `err.constructor.name === "RuntimeError"` checks for OOM (constructor-name string compare because WebAssembly.RuntimeError isn't reliably exposed as a global); regex tightened from over-broad `invalid|format|codec` to anchored alternation + ffmpeg.wasm-specific phrases (`invalid (data|argument)`, `unable to find`, `could not decode`, `malformed`). No new error codes; same verbatim P4.R1 / P4.R2 messages
...
│   │       ├── extract-audio.ts         # `extractAudioFromVideo(file, opts)` — lazy-imports `@ffmpeg/ffmpeg` + `@ffmpeg/util` so the ~25 MB WASM core never lands in the main bundle. Single-threaded core (no SharedArrayBuffer / no COOP-COEP requirement). Runs `-vn -ac 1 -ar 16000 -b:a 32k -f mp3` extraction. Abort listener registered before the first await so cancels during `ffmpeg.load()` are caught. `try/finally` always calls `terminate()`. Output blob constructed via fresh ArrayBuffer copy (TS-lib pedantry: Uint8Array<ArrayBufferLike> is not BlobPart)
...
│   │       └── upload-audio.ts          # `uploadAudioForTranscription(input, opts)` — XMLHttpRequest (not fetch — fetch can't surface upload-progress events). Multipart wire format: `audio` Blob + `video_file_name` + `video_file_type` + `video_file_size` + `duration_seconds`. Single `detachSignalListener()` helper deduplicates abort-listener removal across load / error / timeout / abort terminal paths
│   └── supabase/
...
| POST | `/api/files/transcribe` | PRD-032 Part 1 contract + Part 2 implementation — receives client-extracted audio + original-video metadata, validates, transcribes via real OpenAI Whisper (`experimental_transcribe`) behind `resolveTranscriptionModel()`. Audio is held in memory only; never persisted (P2.R3). Wire format: `multipart/form-data` with `audio` Blob + `video_file_name` + `video_file_type` (in `VIDEO_MIME_TYPES`) + `video_file_size` (≤ 500 MB) + `duration_seconds` (≤ 7200) + optional `session_id` (PRD-032 P2.R7 — auto-persist). Audio capped at 25 MB → 413 (Whisper's hard limit). When `session_id` provided: pre-flight session-access + `MAX_ATTACHMENTS` checks, post-Whisper combined-char check + `createTranscriptAttachment` + `markStale`, response includes the persisted `attachment` row. When omitted: stateless — return transcript only, client persists on save. Returns `{ parsed_content, file_name, file_type, file_size, duration_seconds, source_format: "video_transcript", attachment? }`. Errors: 400 (validation), 413 (audio over limit), 422 (empty transcript per P2.R8 OR combined-char over limit on auto-persist), 500 (config / persist-after-transcribe failure), 502 (upstream Whisper failure after retry exhaustion). | Yes |
