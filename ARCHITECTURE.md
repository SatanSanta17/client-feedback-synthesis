...
**Database tables:** `clients`, `sessions`, `session_attachments`, `session_embeddings`, `themes` (now with `embedding vector(1536)` + HNSW index for the PRD-026 Part 1 prevention guard, and `merged_into_theme_id` FK reserved for PRD-026 Part 3), `signal_themes`, `theme_merge_candidates` (PRD-026 Part 2 — persisted ranked merge candidates per workspace), `theme_merge_dismissals` (PRD-026 Part 2 — admin "not a duplicate" judgments), `master_signals`, `profiles`, `prompt_versions`, `teams`, `team_members`, `team_invitations`, `conversations`, `messages`, `dashboard_insights`, `workspace_notifications` — all with RLS. **RPC functions:** `match_session_embeddings` (vector similarity search), `sessions_over_time` (time-bucketed session counts by week/month for dashboard), `find_theme_candidate_pairs` (PRD-026 Part 2 — pairwise theme similarity above threshold), `fetch_theme_stats` (PRD-026 Part 2 — per-theme assignment / session / client counts + last-assigned timestamp).
...
│       │       ├── candidate-row.tsx            # Single-pair display per P2.R3 — confidence pill (≥0.92 status-success / 0.85–0.92 status-warning / else neutral muted), two side-by-side ThemeSide panels (collapse to stacked on mobile), per-side stats footer with proper singular/plural, shared-keywords hint, score-component breakdown caption; embeds DismissAction; "Merge…" button is a placeholder (disabled, aria-label "Merge — coming in Part 3") for Part 3 wiring
