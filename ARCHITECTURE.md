...
│   │   └── dashboard/
│   │       └── route.ts         # GET — dashboard query route: Zod-validated action (9 actions) + filter params (including confidenceMin), delegates to executeQuery() via anon client (RLS-protected)
│   ├── auth/
│   │   └── callback/
...
│   │   └── _components/
│   │       ├── chart-colours.ts            # Shared chart colour constants for Recharts widgets (brand primary, sentiment, urgency hex maps, theme line palette)
...
│   │       ├── dashboard-content.tsx       # Client coordinator — FilterBar + responsive widget grid (8 widgets, Suspense boundary)
...
│   │       ├── theme-client-matrix-widget.tsx # Theme-client matrix — HTML heatmap grid, opacity-mapped cells, sticky headers, custom tooltip, click drill-down stub
│   │       ├── theme-trends-widget.tsx    # Theme trends — LineChart with local week/month toggle, top-5 default with theme multi-select (Popover+Command), 8-colour palette
│   │       ├── top-themes-widget.tsx      # Top themes — horizontal BarChart, chunk-type breakdown tooltip, 15-theme default with show-all toggle, click drill-down stub
...
│   │   ├── database-query-service.ts # Database query service — action map (13 actions: 7 chat + 3 dashboard + 3 theme widgets) with parameterized Supabase queries, team-scoped; extended filters (clientIds, severity, urgency, granularity, confidenceMin) for dashboard (PRD-020, PRD-021); shared theme helpers (fetchActiveThemeMap, fetchSignalThemeRows) for DRY multi-table joins
