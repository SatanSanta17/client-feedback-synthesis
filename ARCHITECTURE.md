...
│   │   ├── attachment-repository.ts    # AttachmentRepository interface — `AttachmentRow.storage_path: string \| null` (nullable since PRD-032 Part 2 to support transcript-only rows); methods: `create()` (parsed-file insert, requires non-null storage_path), `createTranscript()` (PRD-032 P2 — DB-only insert with storage_path NULL and source_format hardcoded to 'video_transcript' inside the impl), `softDelete()` (returns `string \| null`), upload/remove/getSignedUrl/getStoragePath/getBySessionId/getCountForSession
