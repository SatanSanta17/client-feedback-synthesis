...
| POST | `/api/files/transcribe` | PRD-032 Part 1 stub — receives client-extracted audio + original-video metadata, validates, drains and discards the audio (no retention), returns a deterministic mock transcript. Wire format: `multipart/form-data` with `audio` Blob + `video_file_name` + `video_file_type` (must be in `VIDEO_MIME_TYPES`) + `video_file_size` (≤ 500 MB) + `duration_seconds` (≤ 7200). Audio capped at 50 MB → 413. Returns `{ parsed_content, file_name, file_type, file_size, duration_seconds, source_format: "video_transcript" }`. Part 2 swaps the mock body for a real Whisper / provider-abstracted call without changing the route signature. | Yes |
