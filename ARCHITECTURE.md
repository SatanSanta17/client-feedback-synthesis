...
│   │   │   ├── action-metadata.ts # ACTION_METADATA registry (descriptions per action; `llmToolExposed` flag retained for historical reference but no longer drives behaviour), LOG_PREFIX. **PRD-033 Part 3 cutover** retired the chat-side CHAT_TOOL_ACTIONS tuple, buildChatToolDescription, and assertChatToolActionsInSync — the chat surface now uses primitive tools in `lib/services/chat-tools/` and reaches the dashboard's domain modules through the aggregation/insights services
