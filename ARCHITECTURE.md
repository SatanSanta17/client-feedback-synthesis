...
│   │       ├── video-attachment-section.tsx # PRD-032 Part 1 — owns video item rendering for both call sites (capture form + expanded row). Renders VideoAttachmentCard for in-flight items, an inline completed-transcript card with chevron-toggle for completed items
...
│   │   ├── use-video-attachment.ts # PRD-032 Part 1 — per-attachment state machine: probe → extract → upload → transcribe. Exposes `state: VideoUploadState` and `cancel()`. AbortController propagated through every async stage; abort-checked at every await boundary. Latest-callback refs for `onCompleted`/`onAutoPersisted`/`onError` keep the effect keyed on `[file]` only (otherwise parent re-renders with a new opts object would abort in-flight extractions). Internal `mapToVideoUploadError(err)` regex heuristic — refined in PRD-032 Part 4. **PRD-032 Part 2:** options gain `sessionId` and `onAutoPersisted`; the hook plumbs `sessionId` through to `uploadAudioForTranscription` and branches on `result.attachment` presence (present → `onAutoPersisted(SessionAttachment)`, absent → `onCompleted(VideoTranscriptAttachment)`). Activates the `EMPTY_TRANSCRIPT` code reserved in Part 1's enum via a verbatim-message regex match
