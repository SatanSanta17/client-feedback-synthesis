...
**Database tables:** `clients`, `sessions`, `session_attachments`, `session_embeddings`, `themes` (now with `embedding vector(1536)` + HNSW index for the PRD-026 Part 1 prevention guard, and `merged_into_theme_id` FK reserved for PRD-026 Part 3), `signal_themes`, `master_signals`, `profiles`, `prompt_versions`, `teams`, `team_members`, `team_invitations`, `conversations`, `messages`, `dashboard_insights`, `workspace_notifications` — all with RLS. **RPC functions:** `match_session_embeddings` (vector similarity search), `sessions_over_time` (time-bucketed session counts by week/month for dashboard).
...
│   │   ├── use-unread-count.ts  # Bell badge poller — visibility-aware setTimeout loop (30s while tab is visible, paused when hidden, restarts on focus); returns { count, refetch } (PRD-029 Part 2)
│   │   ├── use-notifications.ts # Bell-list management — paginated fetch from /api/notifications with keyset cursor (cursor + cursorId), optimistic local mark-read mutations (markRowReadLocally / markAllRowsReadLocally), refresh / loadMore; cancelledRef guards post-unmount writes (PRD-029 Part 2)
│   │   └── use-mark-notification-read.ts # Fire-and-forget mark-read POSTs to /api/notifications/[id]/read and /api/notifications/mark-all-read; failures logged but not surfaced — bell stays optimistic, next poll corrects drift (PRD-029 Part 2)
│   ├── types/
...
│   │   └── renderers.ts         # Renderer scaffold — RendererFor<E> / RendererRegistry types, NOTIFICATION_RENDERERS map (Partial<RendererRegistry>, empty in Part 2; Part 3 fills entries), RenderedNotification consumer shape, FALLBACK ({ Bell, "Workspace activity", null }), renderNotification(notification) lookup with Zod payload re-validation as defence-in-depth (PRD-029 Part 2)
