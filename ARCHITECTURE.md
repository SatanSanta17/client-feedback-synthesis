...
│   ├── prompts/
│   │   ├── chat-prompt.ts       # Chat system prompt — tool usage instructions, citation rules, follow-up generation format. CHAT_MAX_TOKENS = 8192 (PRD-031 P3.R1, was 4096 in PRD-020). Rule 5 rewritten in PRD-031 P3.R3 to distinguish conversational vs list-style answers with explicit Rule 7 floor cross-reference
...
│   │   ├── chat-stream-service.ts # Streaming orchestration — tool definitions (searchInsights, queryDatabase), streamText call, SSE event emission, message finalization (PRD-020). PRD-031 Part 3: stopWhen=stepCountIs(10) (was 5), maxOutputTokens wrapped in clampOutputTokens(CHAT_MAX_TOKENS, modelLabel), length-truncation warning block mirrors the existing tool-call truncation block, chat-complete log line carries usage telemetry (input/output/total tokens) and distinguishes (step-truncated) vs (length-truncated)
