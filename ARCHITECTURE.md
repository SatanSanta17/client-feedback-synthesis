...
**Status:** PRD-002 through PRD-010 implemented. PRD-011 (Email + Password Auth, Parts 1–4) implemented. PRD-012 Parts 1–5 (Design Tokens and Typography + DRY Extraction + SRP Component Decomposition + API Route/Service Cleanup + Dependency Inversion) implemented. PRD-013 Parts 1–4 (File Upload Infrastructure + Persistence & Signal Extraction Integration + Past Sessions Attachment Management + Edge Cases & Limits) implemented. PRD-014 Parts 1–4 (Session Traceability & Staleness Data Model + View Prompt on Capture Page + Show Prompt Version in Past Sessions + Staleness Indicators & Re-extraction Warnings) implemented. PRD-015 Part 1 (Public Landing Page) implemented. PRD-016 (Dark Mode) and PRD-017 (Bulk Re-extraction) implemented. PRD-018 Parts 1–2 (Structured Output Migration + Render UI from JSON) implemented. PRD-019 Parts 1–4 (pgvector Setup & Embeddings Table + Chunking Logic + Embedding Pipeline + Retrieval Service) implemented. PRD-020 Parts 1–3 (Sidebar Navigation + Chat Data Model and Streaming Infrastructure + Chat UI Components) implemented. PRD-021 Parts 1–6 (Theme Assignment at Extraction Time + Dashboard Layout, Navigation, and Direct Widgets + Derived Theme Widgets + Qualitative Drill-Down + AI-Generated Headline Insights + Filters and Interactivity) implemented. PRD-022 Parts 1–4 (Settings Accordion + Team Management Page + Extraction Prompt Page + Landing Page Refresh) implemented. PRD-023 Parts 1–3 (Codebase Cleanup: Quick Wins + Shared Route Helpers + Session Orchestrator) implemented. PRD-026 Part 1 (Embedding-Based Prevention at Theme Creation) implemented — every theme now carries a `vector(1536)` embedding (HNSW indexed) populated either by the prevention guard at extraction time or by the one-shot backfill script; semantic-similarity match (default threshold 0.92, env-configurable) collapses synonym proposals onto existing themes before insert; LLM prompt biases new theme names toward umbrella naming as a complementary first line of defense; `merged_into_theme_id` pointer added forward-compat for Part 3. PRD-026 Part 2 (Platform-Suggested Merge Candidates) implemented — `theme_merge_candidates` + `theme_merge_dismissals` tables with RLS, `find_theme_candidate_pairs` + `fetch_theme_stats` RPCs, `theme-candidate-service` with refresh/list/dismiss + composite ranking (`0.5·sim + 0.3·vol + 0.2·rec`) + token-based shared-keywords hint, three admin-gated `/api/themes/candidates*` route handlers (cron deferred per scope decision; staleness handled via stale-on-mount auto-refresh on first visit), admin-only `/settings/themes` page with confidence-pill rendering + per-side stats + dismiss action, sidebar nav conditionally exposes the Themes entry to admins/owners only via `useIsWorkspaceAdmin()`. PRD-026 Part 3 (Admin-Confirmed Theme Merge) implemented — new `theme_merges` audit table with snapshotted theme names + RLS, atomic `merge_themes` PL/pgSQL RPC (locks both theme rows FOR UPDATE, re-points `signal_themes` with conflict-aware delete-then-update, archives + sets `merged_into_theme_id`, cleans up stale candidates/dismissals, writes audit row — all in one transaction), typed-error pipeline (SQLSTATE 22023 → MergeValidationError → 409, P0002 → MergeNotFoundError → 404), `theme-merge-service` with `mergeCandidatePair` + `listRecentMerges`, `WorkspaceCtx` + `matchesWorkspace` helper extracted to a shared `lib/services/workspace-context.ts` module, two new admin-gated `/api/themes/...` routes, merge-dialog with canonical-flip toggle + blast-radius preview + P3.R2 chat-history disclaimer hosted by `themes-page-content.tsx`, recent-merges audit section sibling of the candidates list. PRD-026 Part 4 (Notify Affected Users of Merges) implemented — merge route schedules `theme.merged` notification emit via Next.js `after()` (fire-and-forget; personal workspaces skipped per `team_id NOT NULL` + Decision 26; emit failure logs but doesn't surface as a merge failure), new `recently_merged_themes` dashboard query action backed by workspace-scoped `theme_merges` read with env-tunable window (`THEME_MERGE_INDICATOR_WINDOW_DAYS`, default 7 days), `RecentMergeIndicator` GitMerge icon component rendered next to canonical theme names in the matrix + trends widgets (top-themes BarChart uses a `⤳` suffix marker due to Recharts' string-only tickFormatter, with a caption strip explaining the marker only when at least one displayed theme has it). Audit fixes: `readNumericEnv` helper extracted to `lib/utils/env-numeric.ts` consolidates 4 duplicated env-var-as-number readers across PRD-026 services; `useRecentlyMergedThemes` hook extracted to `app/dashboard/_components/use-recently-merged-themes.ts` consolidates the duplicated useDashboardFetch + Set construction across the three theme widgets. Closes PRD-026. — auth boilerplate, file-upload validation, and the `profiles.can_create_team` query are extracted into `lib/api/` helpers and `team-service.ts`; the post-response embeddings → themes → insights chain on session create / re-extract is owned by `runSessionPostResponseChain` in `lib/services/session-orchestrator.ts` (routes still register it via `after()`). The app is a fully functional team-capable client feedback capture and synthesis platform with a public landing page, vector search infrastructure, Instagram-style hover-to-expand sidebar navigation, complete RAG chat interface (conversations sidebar, message thread with virtualized scrolling, streaming with markdown, citations, follow-ups, in-conversation search, archive read-only mode), and full server-side RAG chat infrastructure (conversations, messages, tool-augmented streaming, LLM title generation). Dual authentication (Google OAuth and email/password with recovery flow), working capture form with AI signal extraction and file attachment upload with server-side persistence, past sessions table with filters/inline editing/soft delete, prompt editor with version history (at `/settings/prompts`), dedicated team management (at `/settings/team`), role-based team access, automatic embedding generation on session save/extraction, and semantic retrieval service with adaptive query classification for downstream RAG and insights features. PRD-029 Parts 1–4 + Part 6 + Part 7 (Workspace Notifications: Data Model & Service + Header Bell UI & Dropdown + Event-Type Catalogue & Renderers + Delivery Mechanism + Session-Scoped Read Filter & Expiry Filter + Per-User Fan-Out & Actor Suppression) implemented — `workspace_notifications` table with RLS + Supabase Realtime publication, closed event-type registry (`lib/notifications/events.ts`), repository, service, four `/api/notifications/*` route handlers, four custom hooks (`use-unread-count`, `use-notifications`, `use-mark-notification-read`, `use-notification-realtime`), renderer registry populated for all three initial event types (`theme.merged` → GitMerge → `/settings/themes`, `supersession.proposed` → Inbox → `/improvements/inbox`, `bulk_re_extract.completed` → CheckCircle → null), and a sidebar-mounted bell with cross-workspace aggregation, optimistic local mark-read, lazy-mounted dropdown content, brand-coloured unread badge (capped display at "9+"), and **Realtime delivery within 1-2 seconds + 5-minute polling fallback** (RLS-on-Realtime filters the event stream per user — no explicit channel filter needed; subscription is shared across the badge and dropdown via the `useNotificationRealtime` hook with a ref-stabilised callback and per-fetch generation counters guarding against stale-response writes). Bell renders specific titles for every registered event; FALLBACK ("Workspace activity") only fires for unregistered event types or malformed payloads. Part 6 layered two narrow filters on top of Parts 1–4: server-side `applyNotExpiredFilter()` excludes rows with `expires_at < now()` from both `executeKeysetListQuery` (listing) and `unreadCount` (badge); client-side, `NotificationBell` captures `sessionStartedAt = Date.now()` once at mount via lazy `useState` and threads it into `useNotifications`, which keeps the full server-returned set in internal `allRows` (so cursor pagination stays valid) and exposes a `useMemo`-derived filtered `rows` where `readAt === null || readAt >= sessionStartedAt` — read notifications survive the current app-mount session and disappear on next hard reload / new tab / browser restart. Soft Next.js client navigation does not reset the session because the bell is sidebar-mounted. No `setInterval` or tick — the filter is static for the session lifetime. Part 7 restructured storage from one-row-per-event to one-row-per-recipient — schema migration `003-fan-out-and-actor-suppression.sql` flips `team_id` to NULLABLE and `user_id` to NOT NULL, drops in-flight broadcast rows, replaces the broadcast-aware RLS policies with a single `user_id = auth.uid()` predicate for SELECT and UPDATE, and rebuilds indexes (the `(user_id, created_at DESC)` and `(user_id) WHERE read_at IS NULL` indexes drop their partial WHEREs since `user_id` is now non-null on every row; the `team_id`-based partials are gone). Service `emit()` gains a recipient-resolution step (new `repo.listActiveTeamMemberIds(teamId)` lookup) and a bulk-insert path (new `repo.bulkInsert(rows[])`); broadcasts fan out to active team members at emit time, optionally minus the new top-level `actorId` (PRD-029 §P7.R3 — actor suppression). Personal-workspace notifications work natively via nullable `team_id`; the merge route's old `if (result.teamId !== null)` guard is gone. Bell, dropdown, hooks, renderers, Realtime subscription unchanged — they were already per-row, which is now per-user by construction. Read isolation is now structural: marking a notification read by user A cannot affect user B's view because A and B have separate rows. PRD-031 Part 1 (Drop Markdown Extraction) implemented — extraction (`extractSignals` in `ai-service.ts`) no longer renders or returns markdown; `POST /api/sessions` and `PUT /api/sessions/[id]` write `structured_notes = NULL` on the extraction path (`createSession` always sets null on insert; `updateSession` sets `resolvedStructuredNotes = null` when `isExtraction === true`, while preserving the legacy markdown-edit branch for pre-PRD-031 rows). The master-signal backend now derives markdown on demand via `composeStructuredNotes()` in `supabase-master-signal-repository.ts` — prefers `renderExtractedSignalsToMarkdown(structured_json)` when JSON is present, falls back to legacy `structured_notes` for pre-PRD-018 rows; queries match either column via the `or("structured_notes.not.is.null,structured_json.not.is.null")` predicate. `SessionDeleteResult` (interface + supabase + mock) carries both columns so the `taintLatestMasterSignal()` gate in `deleteSession` fires when either was populated. The capture flow: extracted post-PRD-031 sessions render via `StructuredSignalView` (JSON path) — the legacy markdown edit toggle in `StructuredNotesPanel` only surfaces when `structured_notes` is non-null. The `useSignalExtraction` hook gained an `initialStructuredJson` option so expanded session rows render correctly when only JSON exists; dirty-tracking switched from "compare current markdown to last extracted markdown" to a `manualEditMade` flag plus a `structuredJson` reference check (`handleSave` overrides the merged row's `structured_json` with the hook's reference so post-save dirty returns to false despite server reparsing). The prompt-editor default for `signal_extraction` (`SIGNAL_EXTRACTION_SYSTEM_PROMPT`) was rewritten as a short advisory guidance template; the legacy markdown-output system prompt content was dead since PRD-018 (it had been demoted to "user guidance" but still contained format directives that confused the model). Per-extraction usage telemetry (`input/output/total` tokens) was added inside `callModelObject()`, covering extraction, theme assignment, and headline insights uniformly. The `sessions.structured_notes` column is retained — populated only for sessions captured before PRD-031 Part 1 and for any user-driven manual markdown edit on legacy rows; hard-delete is parked in the PRD-031 backlog. PRD-031 Part 2 (Positive Signal Chunk Type, Hide-Empty Sections, and Top Wins Dashboard Widget) implemented — extraction schema bumped to v2 (`EXTRACTION_SCHEMA_VERSION = 2`) with a new `positiveSignals` array (using `signalChunkSchema`, `.default([])`); the `ChunkType` literal union gained `"positive_signal"` (TEXT column on `session_embeddings`, no migration); the chunking service emits `chunk_type: "positive_signal"` rows for each positive item, and the master-signal renderer (`renderExtractedSignalsToMarkdown`) emits a "## Positive Signals" section so derived markdown for the master-signal backend includes the new category. The `StructuredSignalView` component renders a "Positive Signals" section between Pain Points and Must-Haves with success-themed severity badges (a new `variant?: "default" | "positive"` prop on `SeverityBadge` reuses existing `--status-success-*` tokens); empty narrative sections are hidden entirely (P2.R6) — the `EmptyState` placeholder was deleted and every narrative `<Section>` is now gated on `chunks.length > 0`, including custom categories filtered per-category. The dashboard gains a **Top Wins** widget alongside Top Themes (`app/dashboard/_components/top-wins-widget.tsx`): same `BarChart` shape, same drill-down lifecycle, but pre-filtered to `chunk_type=positive_signal` via a new `chunkTypes?: string[]` filter on `QueryFilters` (applied in `applyThemeJoinFilters`), success-themed bars (`WIN_BAR_HEX = "#22c55e"`), and **self-hides on empty** (returns `null` from render when the workspace has no positive-signal data). Drill-down from Top Wins forwards `chunkTypes: ["positive_signal"]` through the `ThemeDrillDown` discriminated union (extended with an optional `chunkTypes` field) so the panel scopes to positive-signal rows for the clicked theme. The extraction system prompt gained Rule 12 with worked examples distinguishing positiveSignals from aspirations and from positive competitiveMentions, plus an alignment fix on the schema-version instruction (was lingering at "1" after the bump). Existing `structured_json` rows tagged `schemaVersion: 1` are not re-validated; consumers default-handle missing `positiveSignals` via `?? []`. PRD-031 Part 3 (Looser Chat Response Limits) implemented — chat output cap (`CHAT_MAX_TOKENS` in `lib/prompts/chat-prompt.ts`) raised from 4,096 to 8,192 tokens; tool-call step ceiling in `chat-stream-service.ts` raised from `stepCountIs(5)` to `stepCountIs(10)`; system prompt Rule 5 rewritten to distinguish conversational answers (where brevity is right) from list-style/synthesis answers (where completeness within the budget is right), with explicit "Rule 7 is the non-negotiable floor" cross-reference so the new latitude never overrides "list ALL items the tool returns." A new `lib/services/ai-provider-limits.ts` module exposes `clampOutputTokens(desired, modelLabel)` with a conservative `PROVIDER_OUTPUT_CAPS` floor map (Gemini Flash variants at 8192; OpenAI/Anthropic models with caps ≥ 8192 deliberately unlisted to avoid over-clamping); `chat-stream-service` wraps `maxOutputTokens` with the helper so a configured-but-untested smaller-cap model is silently clamped + warn-logged rather than rejected by the provider. Length-truncation user warning added — when `finishReason === "length"` the streamed content gains an inline "this answer was cut off — try a more focused follow-up" hint, mirroring the existing tool-call-truncation hint and persisted onto the assistant message. Per-turn telemetry: the chat-complete log line now includes `usage: input=… output=… total=…` from `result.usage`, and the existing `(truncated)` literal is renamed to `(step-truncated)` with `(length-truncated)` added so production grep can distinguish the two truncation cases. The 80K conversation-history budget (`DEFAULT_TOKEN_BUDGET` in `chat-service.ts`) is left unchanged — output cap is the bottleneck on response length, not history. Audit fix: pre-existing dead `userId` destructure in `chat-stream-service.ts` (interface field + service body + route call site) removed during the Part 3 audit lint sweep — no behaviour change, smaller `ChatStreamDeps` interface. **Closes PRD-031.** PRD-032 Part 1 (Video Upload and Transcription: Client-side Audio Extraction & Upload) implemented — users can drop `.mp4`/`.mov`/`.webm` into the capture form (capped at 500 MB / 2 hr per P1.R2). The browser probes duration via a hidden `<video preload="metadata">` element, then extracts mono 16 kHz / 32 kbps mp3 audio via `ffmpeg.wasm` running in a Web Worker (lazy-imported via dynamic `import()` so the ~25 MB WASM core never lands in the main bundle; single-threaded core deliberately chosen to avoid COOP/COEP requirements). The audio is uploaded via `XMLHttpRequest` (not fetch — fetch can't surface upload-progress events) to a new `POST /api/files/transcribe` endpoint that authenticates via `requireAuth()`, validates multipart fields with Zod (video name, MIME against `VIDEO_MIME_TYPES`, size, duration), drains and discards the audio (no retention per P2.R3), and returns a deterministic mock transcript — Part 2 will swap the mock body for a real Whisper / provider-abstracted call without changing the wire format. The video file itself never leaves the device. New per-attachment state machine `useVideoAttachment(file, opts)` drives probe → extract → upload → transcribe with `AbortController` propagated through every async stage and abort-checked at every await boundary; latest-callback refs keep the effect keyed on `[file]` only so the cleanup doesn't abort in-flight extractions on parent re-renders. UX safeguards: `useWakeLock(active)` requests a screen wake lock that re-acquires on `visibilitychange` (browsers release on tab hide); `useBeforeUnloadGuard(active)` installs a `beforeunload` listener (the message parameter was dropped because modern browsers ignore the string content); a non-dismissible `<ProcessingVideoBanner>` reminds the user to keep the tab open. **Two-slice state design** (deviation from TRD's merged-list approach): `attachments: ParsedAttachment[]` (existing parsed-file flow, byte-for-byte unchanged) and `videoItems: VideoListItem[]` (in-flight | completed) live as separate slices in both `session-capture-form.tsx` and `expanded-session-row.tsx`; trade-off accepted is that videos and parsed files render in two visual groups instead of one interleaved list, in exchange for zero changes to the existing parsed-file UX. Save and Extract Signals are blocked while any video is processing — toasts "video is still processing" — sidesteps the race where a save mid-extraction would either drop the transcript or save without it. **Part-1-only seam:** video transcripts are NOT yet persisted to `session_attachments` — Part 2 will add the nullable `storage_path` migration and the persistence write; both save handlers `setVideoItems([])` after a successful save to keep the UI honest about what made it to the DB. Completed transcripts ARE included in `composeAIInput()` so `Extract Signals` works end-to-end with mock content (the discriminated `PendingAttachment.video_transcript` variant matches `AttachmentLike` structurally, no signal-extraction changes required). New constants: `VIDEO_MIME_TYPES`, `VIDEO_EXTENSIONS`, `MAX_VIDEO_FILE_SIZE_BYTES`, `MAX_VIDEO_DURATION_SECONDS`, `AUDIO_EXTRACTION_PARAMS` (16 kHz mono / 32 kbps mp3 — speech-to-text-tuned, not playback-tuned), `FFMPEG_CORE_URL`, `FFMPEG_WASM_URL` (self-hosted via `scripts/copy-ffmpeg-assets.mjs` postinstall hook copying `@ffmpeg/core` UMD assets into `public/ffmpeg/` — gitignored, regenerated on every `npm install` including Vercel deploys; 1-year immutable cache header set in `next.config.ts`). New deps: `@ffmpeg/ffmpeg`, `@ffmpeg/util`, `@ffmpeg/core` (all 0.12.x). The `is_edited?: false` literal field on the `video_transcript` discriminated-union variant is reserved for PRD-032 Part 3 (P3.R7 — editable transcripts) — typed but unused in Part 1. PRD-032 Part 2 (Server-side transcription + persistence) implemented — the Part 1 mock-transcript stub at `/api/files/transcribe` is replaced with real OpenAI Whisper via `experimental_transcribe` from the Vercel AI SDK behind a `resolveTranscriptionModel()` resolver mirroring the existing `resolveModel()` pattern (separate `AI_TRANSCRIPTION_PROVIDER`/`AI_TRANSCRIPTION_MODEL` env vars — Whisper is OpenAI-only at the AI SDK layer today; defaults `openai`/`whisper-1` work out of the box). New `transcribeAudio(buffer)` wrapper passes `maxRetries: 0` to the AI SDK so retry/backoff is owned exclusively by the project's `withRetry()` helper (avoids double-retry math), throws `TranscriptionEmptyError` (extends `AIServiceError`) on empty/whitespace-only Whisper output. Audio bitrate `AUDIO_EXTRACTION_PARAMS.bitrate` dropped from `"32k"` to `"24k"` so 2 hr × 24 kbps ≈ 22 MB fits comfortably under Whisper's 25 MB hard limit; route's `MAX_AUDIO_BYTES` tightened from 50 MB → 25 MB to match. Empty-transcript 422s surface the verbatim PRD P2.R8 message; provider failures retry up to 3× via `withRetry` then surface as 502 Bad Gateway (semantically: upstream failure, not our config). The client-side `mapToVideoUploadError` activates the `EMPTY_TRANSCRIPT` code reserved in Part 1's enum via a verbatim-message regex match. Schema migration `001-make-storage-path-nullable.sql` (PRD-032 dir) makes `session_attachments.storage_path` nullable so video transcripts can persist as transcript-only rows; existing parsed-file rows retain non-null paths. Application-layer invariant (no DB CHECK constraint — would couple the schema to the source-format vocabulary): `storage_path IS NULL` for `source_format = 'video_transcript'`, non-null otherwise. Repository surface gains `createTranscript(input: TranscriptAttachmentInsert)` (DB-only, source_format hardcoded inside the repo so callers can't insert under a different format by accident) plus `softDelete` widened to return `string | null`; `attachment-service.deleteAttachment()` skips `removeFromStorage()` when path is null. Two persistence paths complete the part: **manual** for new sessions — `lib/utils/upload-attachments.ts`'s `PendingAttachmentUpload` is now a discriminated union (`kind: "parsed" | "video_transcript"`), `/api/sessions/[id]/attachments` POST branches on `source_format` and the transcript path validates metadata via the new shared `transcriptVideoMetadataSchema` (`lib/schemas/transcript-attachment.ts` — also imported by the transcribe route, single source of truth), enforces `MAX_ATTACHMENTS` and `MAX_COMBINED_CHARS` server-side, calls `createTranscriptAttachment()`, marks the session stale, returns at 201; the capture form's save flow builds a unified `pendingUploads` from `attachments + completedTranscripts` and posts the single batch with `hasAttachments` flag including transcripts. **Auto** for already-saved sessions — `/api/files/transcribe` accepts an optional `session_id` form field; pre-flight (before Whisper) `requireSessionAccess` + `MAX_ATTACHMENTS` count check saves a Whisper round-trip when the session is already full, post-Whisper combined-char check + `createTranscriptAttachment()` + `markStale()` happen inside the same handler before responding so the transcript survives client disconnect mid-Whisper (P2.R7); response includes the persisted attachment row. `useVideoAttachment` plumbs `sessionId` and `onAutoPersisted` through to `uploadAudioForTranscription`, branches on `result.attachment` presence — present → `onAutoPersisted(SessionAttachment)`, absent → `onCompleted(VideoTranscriptAttachment)`. `useVideoItemsState` accepts an options-bag with `sessionId` + `onAutoPersisted`; when `sessionId` is set, `handleVideoAutoPersisted` is exposed (removes the videoItem entirely and forwards the saved row to the consumer); when absent, the auto-persist surface gates undefined down through the entire prop chain (Section → Card → hook), so the capture-form path is unreachable — ISP working as designed. `expanded-session-row.tsx` opts in (passes `session.id` + `onAutoPersisted` that pushes into `savedAttachments`); `session-capture-form.tsx` does not. `saved-attachment-list.tsx` defensive patch hides the download button when `source_format === "video_transcript"` (no blob to download); the rest of the Part 3 visual differentiation lands in Part 3. PRD-032 Part 3 (Video transcript UX in sessions: visual differentiation + editable transcripts) implemented — most of P3 was already shipped as side-effects of Parts 1–2 (P3.R2 no-download, P3.R3 inline view, P3.R4 paperclip count, P3.R5 re-extraction, P3.R6 add-to-existing-session); the genuinely-new pieces are **P3.R1** (visual sub-label "Transcript only — original video not stored" on transcript rows) and **P3.R7** (editable transcripts, exclusive to `source_format = "video_transcript"`, with an "edited" badge and last-edited timestamp tooltip). Schema migration `002-add-transcript-edit-tracking.sql` adds a single `last_edited_at TIMESTAMPTZ NULL` column to `session_attachments` — NULL means never edited, non-null means edited (timestamp shown in tooltip via `formatRelativeTime`). No CHECK constraint coupling to `source_format` — application-layer rule, brittle to vocabulary changes. New `updateTranscript(attachmentId, parsedContent)` repository method UPDATEs `parsed_content` + sets `last_edited_at = now()`, with defence-in-depth `eq("source_format", "video_transcript")` predicate so a stale client racing past the route's check still hits a no-op. New `updateTranscriptAttachment(repo, attachmentId, parsedContent)` service function. New `PATCH /api/sessions/[id]/attachments/[attachmentId]` method alongside the existing DELETE — auth via `requireSessionAccess`, JSON body parse, P4.R6 verbatim empty-content rejection (`"Transcript can't be empty. Use Remove if you want to discard this attachment."`), single `getAttachmentsBySessionId` fetch covers existence + source_format + combined-char checks (uniform 404 for not-found-OR-not-a-transcript — doesn't leak whether the row exists vs. isn't editable), delegates to `updateTranscriptAttachment`, marks session stale, returns the updated row at 200. POST attachments transcript branch reads optional `is_edited` form field via strict `=== "true"` — when received, the repo writes `last_edited_at = now()` on insert so the badge appears on first render. Two new shared components: `TranscriptEditor` (controlled inline editor — Save gated on empty/unchanged/saving; P4.R6 inline error matches the server message verbatim; auto-focuses; `font-mono text-xs` matches the View-content panel styling) and `EditedBadge` (small "edited" pill; optional `timestamp?: string | null` prop wraps in `<span title={`Edited ${formatRelativeTime(...)}`}>` when set; native `title` is the project's existing pattern — no shadcn `<Tooltip>` primitive installed). `SavedAttachmentList` is the primary edit surface — adds the sub-label, the EditedBadge when `last_edited_at !== null`, the Pencil Edit button (transcript-only when `canEdit`), and the inline editor (replaces the chevron-toggled content panel during edit; Cancel is the single exit path). The "video_transcript" raw text format-badge is hidden on transcript rows because the icon + sub-label + EditedBadge already communicate format/state. `useVideoItemsState` gains `handleTranscriptEdited(id, parsedContent)` for the pre-save edit case (mutates the videoItem's `data.parsed_content` + flips `data.is_edited = true`); `CompletedTranscriptCard` in `VideoAttachmentSection` gains the same Pencil Edit button + inline editor + EditedBadge (with `timestamp={null}` because persistence hasn't happened yet); both save flows (capture form + expanded row) propagate `is_edited: t.is_edited === true` on the upload payload's transcript variant. The `is_edited?: boolean` field on `VideoTranscriptAttachment` (Part 1's `is_edited?: false` literal reservation) is now wired end-to-end. Audit-bonus: extracted `ATTACHMENT_ROW_FIELDS` constant in the Supabase repo (now used in 4 SELECT statements); fixed a pre-existing `completedTranscripts` missing-dep warning on `expanded-session-row.tsx`'s `handleSave` `useCallback` (Part 2 Increment 2.3 introduced it, 2.6's audit missed it). PRD-032 Part 4 (Edge cases and limits) implemented — five of six PRD requirements (P4.R1, P4.R2, P4.R3, P4.R5, P4.R6) were already shipped via Parts 1–3 and verified in this part's audit; the genuinely-new pieces are **P4.R4** (sequential video processing — concurrency-1 queue at `<VideoAttachmentSection>` level, with queued items rendered as `Clock`-icon "Waiting…" placeholder cards) and a **heuristic refinement on `mapToVideoUploadError`** to catch `RangeError` and `WebAssembly.RuntimeError` explicitly for OOM detection plus tighter ffmpeg.wasm-specific patterns (`invalid (data|argument)`, `unable to find`, `could not decode`, `malformed`) for codec failures. The queue lives at the section level — `firstInFlightIndex = items.findIndex((i) => i.status === "in_flight")` once per render; only that item gets a fully-mounted `<VideoAttachmentCard>` (which auto-starts ffmpeg.wasm on mount), other in-flight items render as inline `<QueuedVideoCard>` placeholders. Queue advances naturally on transitions out of `in_flight`: completed → status flips, re-render → next item active; auto-persisted → item removed entirely (Part 2 behaviour); cancelled → item removed; errored → user dismisses via Remove (intentional gate — surfaces failures, no silent advance). **No state-shape change**, no hook change, no prop changes on the section — queue policy is purely a render-time decision. Heuristic refinement: explicit `instanceof RangeError` + `err.constructor.name === "RuntimeError"` checks (string compare because WebAssembly.RuntimeError isn't reliably exposed as a global across runtimes); regex tightened from over-broad `invalid|format|codec` to anchored alternation + ffmpeg.wasm-specific phrases. **No new error codes** — `EXTRACTION_OOM` / `EXTRACTION_FAILED` continue to map to the same verbatim P4.R1 / P4.R2 user messages from Part 1. **Closes PRD-032.** PRD-033 Parts 1–3 (Agentic Chat — Primitive Tool Surface) implemented — the chat tab now uses an 11-tool primitive surface (`lib/services/chat-tools/`: list_clients / list_sessions / list_themes / semantic_search / fetch_session_content / fetch_signals / aggregate / time_series / summarise_sessions / insights_latest / insights_history) chained by the model itself, replacing the previous two-tool surface (searchInsights + queryDatabase with a 15-action enum). **Part 1** built the discovery/retrieval/aggregation primitives, switched retrieval to hybrid (vector + Postgres `tsvector` FTS fused via RRF, `k=60` default — closes the exact-term gap with no paid reranker), added two migrations (`session_embeddings.chunk_text_tsv` generated column + GIN index; `match_session_embeddings_fts` RPC), the dedicated `ChatQueryRepository` (deviation from initial TRD plan, documented), per-tool Zod filter contracts that retire the shared 8-field filter bag and its cue-matching sanitiser, and a DIY TS eval harness (`scripts/run-eval.ts` + `docs/033-agentic-chat/eval/queries.json` with 16 queries spanning quantitative/qualitative/discovery/hybrid/exact-term; LLM-as-judge prompt versioned at `v1`; subsequence-matching for tool-routing accuracy). **Part 2** added `summarise_sessions` (map-reduce against a separately-configured cheap model via `SUMMARY_AI_*` env vars — independent of `AI_PROVIDER` per PRD P2.R2, no fallback; bounded concurrency 5, fan-out cap 50, per-row error isolation with sanitised model-facing errors, four partial-coverage signals `capReached`/`budgetReached`/`outOfScopeCount`/per-row `error`, no content retention), plus 10 P2-tagged eval queries and a `partOfBreakdown` aggregation in the runner. **Part 3** was the cutover commit: rewrote the chat system prompt to v2 (`CHAT_PROMPT_VERSION = "v2"`; six-section structure with explicit routing patterns and the `__BUDGET_EXHAUSTED__` sentinel contract); deleted ~150 lines of old tool builders + cue maps + `lastUserMessage` plumbing from `chat-stream-service.ts`; wired the new tools wrapped by the per-turn cost circuit breaker (`chat-cost-budget.ts`, default 100k tool-result tokens, one-shot first-call rule, sentinel-payload injection past budget) and the provider-aware prompt cache (`chat-prompt-cache.ts`, Anthropic explicit `cacheControl`, OpenAI automatic, Google deferred no-op); retired `CHAT_TOOL_ACTIONS` / `buildChatToolDescription` / `assertChatToolActionsInSync` from `database-query/action-metadata.ts` (the dashboard's action surface is unaffected); replaced one starter question with "Summarise everything our most active client has told us this quarter" to surface the new capability. Per-turn `chat-complete` log line now carries cache-hit/miss input tokens, tool-result tokens, budget-exceeded flag, prompt version. **Three remaining steps deferred to the cutover PR**: step-cap review (expected no raise), eval-gate run against staging (≥ 90% answer-correctness AND ≥ 90% tool-routing accuracy, zero regressions, verifying the sentinel contract end-to-end), and 1–2 weeks of post-cutover production observation with `git revert` as the rollback lever (no feature flag — early-stage product). **Closes PRD-033.** **Note:** the master-signal backend (API route, service, repository, prompt, `master_signals` table) remains wired but the `/m-signals` UI page has been retired — see "Master Signal (retained backend, retired UI)" below.

**Core features live:**
- Public landing page at `/` with hero, product showcase (framed dashboard screenshot with floating callout chips for Sentiment trends / Theme matrix / RAG chat — fades up on scroll with staggered chip reveal), bento-style features grid (AI Signal Extraction as hero card with row-span-2 and corner glow + Capture / Insights / Ask Your Data in remaining cells), "Built for" personas strip (Founders / Sales / Customer Success / Product Managers), how-it-works flow, bottom CTA, and Contact section with working form (name/email/message → service-role insert into `contact_submissions` + Resend/Brevo email notification with reply-to set to submitter; per-IP rate limit 5/10min, 10KB content-length cap, off-screen honeypot field for bot suppression). Authenticated users auto-redirect to `/dashboard`; new accounts with no sessions redirect to `/capture`.
...
│   ├── _components/
│   │   ├── landing-page.tsx                # Client component — auth-aware landing page shell (nav, hero, how-it-works, bottom CTA, footer); composes the new section components
│   │   ├── landing-product-showcase.tsx    # Showcase section — framed dashboard screenshot with floating callout chips, scroll-reveal animation
│   │   ├── landing-features-bento.tsx      # Asymmetric bento features grid — hero card (AI Signal Extraction, row-span-2 with corner glow) + three standard cards; staggered scroll-reveal
│   │   ├── landing-personas.tsx            # "Built for" personas strip — compact (not full-viewport) section with four icon+label+statement rows (Founders, Sales, CS, PMs)
│   │   ├── landing-contact-section.tsx     # Contact section — full-viewport, two-column desktop (pitch + alt-channels left, form right); composes LandingContactForm
│   │   ├── landing-contact-form.tsx        # 3-field form (Name/Email/Message) — react-hook-form + zod, FormState discriminated union (idle/submitting/success/error), honeypot input, inline success swap, toast on error
│   │   └── landing-footer.tsx              # Three-column landing-page footer — brand+credits / quick-link anchors / social icons + theme toggle. Owns SOCIAL_LINKS and QUICK_LINKS constants (PRD-030 Part 4)
│   ├── favicon.ico
│   ├── api/
│   │   ├── ai/
│   │   │   ├── extract-signals/
...
│   │   ├── contact/
│   │   │   └── route.ts         # POST — public landing-page contact form (Zod validate, in-memory per-IP rate limit 5/10min, honeypot absorb, 10KB content-length cap, persist + notify orchestration). Excluded from middleware auth redirect via `pathname === "/api/contact"` in the public-route check (PRD-030 Part 3).
│   │   ├── files/
│   │   │   ├── parse/
│   │   │   │   └── route.ts     # POST — stateless file parse (multipart/form-data) — returns parsed content + source format
│   │   │   └── transcribe/
│   │   │       ├── route.ts     # POST — per-chunk audio transcription via Whisper. Stateless (no session_id). `maxDuration = 60`. See routes table for full contract (2026-05-13 chunked-transcribe)
│   │   │       └── finalize/
│   │   │           └── route.ts # POST — persist stitched transcript as a session_attachments row + markStale. JSON body. `maxDuration = 60`. (2026-05-13 chunked-transcribe)
│   │   ├── invite/
│   │   │   └── [token]/
│   │   │       └── accept/
...
│   │       ├── dashboard-content.tsx       # Client coordinator — FilterBar + InsightCardsRow + PreviousInsights + responsive widget grid (9 widgets), drill-down state + DrillDownPanel, cross-widget filtering (applyWidgetFilter), freshness tracking (ref+context), screenshot export (dashboardRef), Suspense boundary
...
│   │       ├── top-wins-widget.tsx        # Top wins — horizontal BarChart pre-filtered to chunk_type=positive_signal via extraParams, success-themed bars (WIN_BAR_HEX), self-hides via early `null` return when the workspace has no positive-signal data, drill-down forwards chunkTypes through ThemeDrillDown so the panel scopes to positive-signal rows. Near-duplicate of top-themes-widget shape with four deliberate differences (filtered query / colour / self-hide / drill-down chunk-type forwarding) — PRD-031 P2.R9
...
│   │       └── use-recently-merged-themes.ts # PRD-026 Part 4 — wraps useDashboardFetch + Set construction for the `recently_merged_themes` action; returns `Set<canonicalThemeId>` consumed by all three theme widgets to render the GitMerge indicator (audit Fix B in P4)
│   ├── capture/
...
│   │       ├── processing-video-banner.tsx # PRD-032 Part 1 — non-dismissible role="status" banner shown while any video is processing; reminds user to keep the tab open. Self-gated on `active` so callers include unconditionally
│   │       ├── prompt-version-filter.tsx  # Prompt version filter dropdown — fetches distinct versions, "Default prompt" for null
│   │       ├── edited-badge.tsx           # PRD-032 Part 3 — small "edited" pill rendered next to a transcript's file name when modified. Optional `timestamp?: string \| null` prop wraps in a native `<span title={...}>` showing `formatRelativeTime(timestamp)` when set; renders alone (no tooltip) when absent (used by pending transcripts pre-save). Native `title` is the project's pattern — no shadcn Tooltip primitive
│   │       ├── saved-attachment-list.tsx  # Displays persisted attachments with download, delete (with confirmation when signals exist), and view parsed content toggle. **PRD-032 Part 3:** transcript rows show the "Transcript only — original video not stored" sub-label, an `EditedBadge` when `last_edited_at !== null`, and a Pencil Edit button when `canEdit`. Click-Edit replaces the chevron-toggled content panel with the inline `TranscriptEditor`; on save, calls `PATCH /api/sessions/[id]/attachments/[attachmentId]` and forwards the updated row to the parent via `onEdited`. The "video_transcript" format-badge text is hidden — icon + sub-label + EditedBadge already communicate format/state
│   │       ├── transcript-editor.tsx      # PRD-032 Part 3 — shared inline editor used by both the saved-transcript path (PATCH endpoint) and the pending-transcript path (client-state mutation in CompletedTranscriptCard). Controlled textarea + Cancel/Save; Save gated on empty/unchanged/saving; P4.R6 verbatim empty-content inline error; auto-focuses on mount; `font-mono text-xs` matches the View-content panel styling. Pure presentational — save/cancel decisions live in the parent
│   │       ├── session-capture-form.tsx   # Coordinator — react-hook-form, submit, extract; composes attachment/notes subcomponents. PRD-032 Part 1: video state + handlers + wake lock + beforeunload guard owned by `useVideoItemsState("[SessionCaptureForm]")`; processing banner mounted at form header; Save and Extract Signals are blocked while any video is processing
...
│   │       ├── video-attachment-card.tsx  # PRD-032 Part 1 — per-video card driven by `useVideoAttachment`. State-specific render: queued/probing/transcribing → spinner; extracting/uploading → progress bar; error → red card with Remove button; completed/cancelled → null (parent transitions). Cancel button calls hook abort + parent removal in one handler to avoid one-frame "Cancelled" flash. **2026-05-13 (chunked transcribe):** StatusLine reads `chunksDone` / `totalChunks` from `VideoUploadState` and renders "Transcribing chunk X/Y…" when `totalChunks > 1`; single-chunk videos still show the original "Transcribing…" with no counter wart
│   │       ├── video-attachment-section.tsx # PRD-032 Part 1 — owns video item rendering for both call sites (capture form + expanded row). Renders VideoAttachmentCard for in-flight items, an inline completed-transcript card with chevron-toggle for completed items. **PRD-032 Part 4 (P4.R4):** concurrency-1 queue — `firstInFlightIndex = items.findIndex((i) => i.status === "in_flight")` once per render; only that item gets a fully-mounted VideoAttachmentCard (which auto-starts ffmpeg.wasm on mount), other in-flight items render as inline `QueuedVideoCard` placeholders (Clock icon + file-type icon + "Waiting…" sub-label + Cancel button). Queue advances naturally on `in_flight` transitions (completed/auto-persisted/cancelled/errored-then-dismissed). No state-shape change, no hook change — purely a render-time decision in the section
...
│   │   ├── use-beforeunload-guard.ts # PRD-032 Part 1 — installs a `beforeunload` listener while `active`. Modern browsers ignore the message string and show their own prompt (the original `message` parameter was dropped in the Part 1 audit). Next.js client-side navigation is a known browser-API gap and is documented in the file
...
│   │   ├── use-video-attachment.ts # PRD-032 Part 1 — per-attachment state machine: probe → extract → upload → transcribe. Exposes `state: VideoUploadState` and `cancel()`. AbortController propagated through every async stage; abort-checked at every await boundary. Latest-callback refs for `onCompleted`/`onAutoPersisted`/`onError` keep the effect keyed on `[file]` only (otherwise parent re-renders with a new opts object would abort in-flight extractions). **PRD-032 Part 2:** options gain `sessionId` and `onAutoPersisted`; the hook branches on `result.attachment` presence (present → `onAutoPersisted(SessionAttachment)`, absent → `onCompleted(VideoTranscriptAttachment)`). Activates the `EMPTY_TRANSCRIPT` code reserved in Part 1's enum via a verbatim-message regex match. **PRD-032 Part 4 (P4.R1, P4.R2):** `mapToVideoUploadError(err)` heuristic refined — explicit `instanceof RangeError` + `err.constructor.name === "RuntimeError"` checks for OOM (constructor-name string compare because WebAssembly.RuntimeError isn't reliably exposed as a global); regex tightened from over-broad `invalid|format|codec` to anchored alternation + ffmpeg.wasm-specific phrases (`invalid (data|argument)`, `unable to find`, `could not decode`, `malformed`). No new error codes; same verbatim P4.R1 / P4.R2 messages. **2026-05-13 (chunked transcribe):** delegates to `transcribeChunkedAudio` instead of the single-call helper. After `probeVideoMetadata` returns, passes `duration_seconds` into `extractAudioFromVideo` (which now segments the audio into ~12 min MP3 chunks), then orchestrates parallel per-chunk POSTs. State transitions stay the same shape but `uploading.progress` is the aggregate fraction across all chunks, and `transcribing` carries `chunksDone`/`totalChunks`. Finalize routing is unchanged — `result.attachment` is populated iff the server's `/api/files/transcribe/finalize` call succeeded
│   │   ├── use-video-items-state.ts # PRD-032 Part 1 (extracted in audit) — owns the `videoItems` state slice for any surface hosting a FileUploadZone with onVideoSelected. Exposes `videoItems`, derived `anyVideoInFlight` / `completedTranscripts` / `transcriptChars`, four handlers (selected/completed/error/remove), and `reset()`. Mounts `useWakeLock` and `useBeforeUnloadGuard` internally so the side-effects move with the state, not duplicated at every call site. **PRD-032 Part 2:** options-bag form (`{ logPrefix, sessionId?, onAutoPersisted? }`) is the only accepted shape after the Part 2 audit removed the back-compat string-arg variant. When `sessionId` is set, additional `handleVideoAutoPersisted` callback is exposed — removes the videoItem entirely (auto-persist replaces it with a `savedAttachments` row in the parent) and forwards the saved row via the consumer's `onAutoPersisted` callback. When `sessionId` is absent, the auto-persist surface gates undefined down through Section → Card → per-attachment hook, so the manual save flow is the only reachable path
│   │   ├── use-wake-lock.ts # PRD-032 Part 1 — requests a screen wake lock while `active`; re-acquires on `visibilitychange` (browsers release on tab hide); releases on cleanup. Uses standard `WakeLockSentinel`/`Navigator.wakeLock` lib.dom types (no shim). Logs warn on unavailable API or acquire failure
│   │   ├── use-scroll-reveal.ts # IntersectionObserver-driven one-shot reveal — returns `[scrollRef, isVisible]` tuple; observer disconnects after first intersection so animations do not retrigger. Tuple shape (vs. `{ ref, isVisible }` object) avoids `react-hooks/refs` linter heuristic that misidentifies `.ref` access on hook returns as `useRef.current` access. Consumed by every animated landing-page section (how-it-works steps, product showcase, features bento, personas).
...
│   │   └── video-attachment.ts # PRD-032 Part 1 — `PendingAttachment` discriminated union (`kind: "parsed" | "video_transcript"`, with `is_edited?: false` literal field reserved for PRD-032 P3.R7 editable transcripts). `VideoTranscriptAttachment` extracted alias. `VideoListItem` (in-flight | completed; error state lives inside the hook). `VideoUploadState` state-machine union. `VideoUploadError` flat shape with `VideoUploadErrorCode` enum covering all eight codes including `EMPTY_TRANSCRIPT` and `UNSUPPORTED_BROWSER` reserved for Parts 2/4. **2026-05-13 (chunked transcribe):** `uploading` and `transcribing` variants gain optional `chunksDone` / `totalChunks` (absent for single-chunk videos so the UI can suppress the `1/1` counter)
...
│   ├── prompts/
│   │   ├── chat-prompt.ts       # **System prompt v2 (PRD-033 Part 3 cutover)** — `buildSystemPrompt({ date })` returns the stable prompt body with today's date substituted; `CHAT_PROMPT_VERSION = "v2"` is exported and recorded in message metadata + chat-complete log line. Sections: tool catalogue (11 primitive tools), routing patterns (list→fetch/summarise→synthesise; multi-query semantic_search; prefer summarise_sessions over fetch_session_content when N > ~10), grounding rules (cite client + date inline; no hallucinated citations; quantitative answers no-cite), output format (list-completeness floor, markdown), partial-coverage handling (budgetReached / capReached / outOfScopeCount / per-row error mention rules), `__BUDGET_EXHAUSTED__` sentinel contract (model must stop calling tools and surface a "too broad" suggestion in plain language). CHAT_MAX_TOKENS = 8192 unchanged from PRD-031 P3.R1
...
│   │   ├── contact-schema.ts      # Public landing-page contact form schema — name (1–100), email (valid+254), mobile (optional, ≤30), message (1–2000), website honeypot (any string, route absorbs filled). Shared between LandingContactForm and /api/contact route (PRD-030 Part 3 + mobile follow-up)
│   │   └── transcript-attachment.ts # PRD-032 Part 2 — `transcriptVideoMetadataSchema` Zod validator for the original-video metadata fields (file_name, file_type against VIDEO_MIME_TYPES, file_size ≤ 500 MB, duration ≤ 7200 s). Imported by both `/api/files/transcribe` POST and `/api/sessions/[id]/attachments` POST (transcript branch) — single source of truth so the two routes can't drift
│   ├── repositories/
...
│   ├── services/
│   │   ├── ai-service.ts        # AI service — public generics callModelText() and callModelObject<T>() with retry; extractSignals(), synthesiseMasterSignal(), generateConversationTitle(); provider-agnostic via Vercel AI SDK. **PRD-032 Part 2:** new `transcribeAudio(buffer)` wrapper using `experimental_transcribe`, behind a `resolveTranscriptionModel()` resolver (separate `AI_TRANSCRIPTION_PROVIDER`/`AI_TRANSCRIPTION_MODEL` env vars; Whisper/openai-only at the AI SDK layer today). Reuses the existing `withRetry()` helper for retry/backoff (passes `maxRetries: 0` to the AI SDK to defer all retry behaviour). Throws `TranscriptionEmptyError` (extends `AIServiceError`) on empty/whitespace-only output
│   │   ├── chat-service.ts      # Chat service — conversation CRUD, message CRUD, buildContextMessages() with 80K-token budget (PRD-020)
│   │   ├── chat-stream-service.ts # Streaming orchestration — builds the ChatToolContext (workspace, repos, status emitter, cost tracker), wires the agentic primitive tool surface via createChatTools(ctx) wrapped by the per-turn cost budget tracker, applies prompt-cache markers via applyPromptCacheMarkers, streamText call, SSE event emission, message finalization. **PRD-033 Part 3 cutover** — replaced searchInsights + queryDatabase + sanitization layer with the 11-tool primitive surface; chat-complete log line now carries cache-hit/miss telemetry, tool-result-tokens, budget-exceeded flag, promptVersion alongside the existing usage stats
│   │   ├── ai-provider-limits.ts  # PRD-031 Part 3 P3.R5 — clampOutputTokens(desired, modelLabel) with conservative PROVIDER_OUTPUT_CAPS floor map (Gemini Flash variants at 8192). Returns desired unchanged for unlisted models so newer/larger-cap models aren't over-clamped; logs a warning when a clamp fires
...
│   │   │   ├── action-metadata.ts # ACTION_METADATA registry (description per action), LOG_PREFIX. **PRD-033 Part 3 cutover** retired the chat-side CHAT_TOOL_ACTIONS tuple, buildChatToolDescription, assertChatToolActionsInSync, and the `llmToolExposed` flag on each entry (deleted in the end-of-Part-3 audit) — the chat surface now uses primitive tools in `lib/services/chat-tools/` and reaches the dashboard's domain modules through the aggregation/insights services
...
│   │   ├── chat-cost-budget.ts     # **PRD-033 Part 3** — `createCostBudgetTracker(budgetTokens, opts)` per-turn cost circuit breaker. Wraps each tool's execute() with a guard; one-shot first-call rule (first tool always lands, subsequent calls past budget get the `__BUDGET_EXHAUSTED__` sentinel payload instead of running). System prompt v2 teaches the model to recognise the sentinel, stop calling tools, and synthesise from earlier results with a user-facing "too broad — try narrowing" message
│   │   ├── chat-prompt-cache.ts    # **PRD-033 Part 3** — `applyPromptCacheMarkers(systemPrompt, history)` adds provider-specific cache markers. Anthropic: explicit `cacheControl: { type: "ephemeral" }`. OpenAI: automatic, no marker (read `cachedInputTokens` from response usage). Google: deferred (no-op until Google becomes an active provider). `readCacheTelemetry(usage)` extracts cache-hit/miss input token counts; falls back across SDK versions; emits one-time warning if not exposed
...
│   │   │   ├── aggregation-service.ts      # aggregate / timeSeries — thin adapter mapping (entity, groupBy) tuples to existing database-query QueryAction enum per PRD-033 § P1.R3 mapping table; one explicit reshape branch per action (no generic key-scanning — audit fix 2026-05-11). Multi-dim groupBy supported for `theme×client matrix`; `client×severity` is not routed (the dashboard's `client_health_grid` handler returns a sentiment/urgency snapshot, no per-row severity) and falls through to a loud "Unsupported aggregate" throw
...
│   │   ├── chat-tools/         # **PRD-033 Parts 1+2+3** — Vercel AI SDK tool factories for the agentic chat surface. Wired to streaming via chat-stream-service.ts as of Part 3 cutover. Each tool is one file with a Zod input schema (per-tool filter contract — no shared filter bag), tool description tuned for routing accuracy, and an execute() that delegates to the corresponding chat-tool-service
│   │   │   ├── index.ts                       # createChatTools(ctx) registry — returns Record<toolName, Tool> with all 11 tools (Parts 1+2 set, including summarise_sessions)
│   │   │   ├── shared/
│   │   │   │   ├── tool-context.ts            # ChatToolContext type — { workspace, chatQueryRepo, embeddingRepo, supabaseClient, emitStatus }. The Part 1/2 forward-compat hooks (cheapModel, recordToolResultTokens) were both retired: cheapModel is resolved inside summarise-sessions-service.ts (Part 2 TRD § 2.7), and the cost circuit breaker records tokens internally inside its wrap() closure (Part 3 audit retired the redundant hook)
│   │   │   │   └── chunk-type-enum.ts         # Shared Zod enum mirroring `ChunkType`. Consumed by every tool that accepts a `chunkTypes` filter (semantic_search, list_sessions, fetch_signals) — single source of truth so adding a chunk type is a one-file change
...
│   │   │   ├── semantic-search-tool.ts        # Retrieval — semantic_search (hybrid via retrieval-service); for similarity questions, NOT completeness questions
...
│   │   ├── email-service.ts     # Provider-agnostic email sending — sendEmail(), resolveEmailProvider(), Resend + Brevo adapters; EmailPayload supports optional replyTo (PRD-030 Part 3)
...
│   │   ├── env-numeric.ts         # readNumericEnv(varName, fallback, { min, max, kind, logPrefix }) — single source for env-var-as-number reads with bounds/kind validation + warning fallback; replaces 4 duplicated readXFromEnv helpers across PRD-026 services (audit Fix A in P4)
...
│   │   ├── upload-attachments.ts  # Uploads pending attachments to a session (shared by capture form + expanded row). **PRD-032 Part 2:** `PendingAttachmentUpload` is now a discriminated union (`kind: "parsed" | "video_transcript"`); both kinds POST to `/api/sessions/[id]/attachments` but ship different FormData shapes — parsed sends file Blob + parsed_content + source_format; transcript sends source_format=video_transcript + parsed_content + video_file_name/type/size + duration_seconds
│   │   └── video/                 # PRD-032 Part 1 — framework-agnostic video utilities consumed by useVideoAttachment
│   │       ├── can-process-video.ts     # `canProcessVideoInBrowser()` — single capability gate (window/Worker/WebAssembly/<video> element). Safe in SSR (returns ok:false reason:"server" when window is undefined). Internal `VideoCapabilityResult` type — return-type only, intentionally unexported
│   │       ├── extract-audio.ts         # `extractAudioFromVideo(file, totalDurationSeconds, opts)` — lazy-imports `@ffmpeg/ffmpeg` + `@ffmpeg/util` so the ~25 MB WASM core never lands in the main bundle. Single-threaded core (no SharedArrayBuffer / no COOP-COEP requirement). Runs `-vn -ac 1 -ar 16000 -b:a 24k -f segment -segment_time 720 -segment_format mp3 -reset_timestamps 1 chunk%03d.mp3` in a single ffmpeg pass — outputs N MP3 chunks where N = `ceil(totalDurationSeconds / TRANSCRIPTION_CHUNK_SECONDS)`. Returns `ExtractedAudioChunk[]` (single-chunk videos return length-1 array; no special-case path). Each chunk: `{ blob, mimeType, extension, index, durationSeconds }`. Abort listener registered before the first await so cancels during `ffmpeg.load()` are caught. `try/finally` always calls `terminate()`. Output blobs constructed via fresh ArrayBuffer copy (TS-lib pedantry: Uint8Array<ArrayBufferLike> is not BlobPart)
│   │       ├── probe-video-metadata.ts  # `probeVideoMetadata(file, signal?)` — reads duration via a hidden `<video preload="metadata">` element. No decoder, no ffmpeg. Cheap pre-check that rejects oversize/over-duration uploads before paying the WASM core load cost
│   │       └── upload-audio.ts          # **2026-05-13 (chunked transcribe):** exports `transcribeChunkedAudio(chunks, meta, callbacks)` — top-level orchestrator. Fans per-chunk POSTs out with `MAX_TRANSCRIPTION_CONCURRENCY=3` via an internal `runWithConcurrency` worker pool. Per-chunk wire format is unchanged multipart (`audio` Blob + `video_file_name` + `video_file_type` + `video_file_size` + `duration_seconds`) but `session_id` is never sent — persistence has moved to the finalize route. An internal `AbortController` composes the parent signal with one that aborts siblings on any chunk failure so we don't pay for transcription that's about to be discarded. After all chunks return, joins `parsed_content` with `\n` separators (no overlap/dedup in v1). When `meta.sessionId` is set, POSTs the stitched transcript to `/api/files/transcribe/finalize` and surfaces the saved `attachment` in the result. Callbacks: `onAggregateUploadProgress(fraction)` (sum of per-chunk fractions ÷ N), `onChunkTranscribed(done, total)`. Per-chunk uploads use XMLHttpRequest (fetch can't surface upload-progress); the single `finalizeTranscript` call uses `fetch(..., { signal })`. Single `detachSignalListener()` helper deduplicates abort-listener removal across load / error / timeout / abort terminal paths
│   └── supabase/
...
│   ├── run-eval.ts                  # **PRD-033 Part 1** (shim filled in 2026-05-11) — CLI eval runner (`npm run eval:chat -- [--query=Q-0xx] [--surface=new]`). Loads docs/033-agentic-chat/eval/queries.json, invokes the chat surface for each query, scores answer correctness via LLM-as-judge (judge-prompt.ts), and tool-routing accuracy via subsequence match. Writes per-query + aggregate report to docs/033-agentic-chat/eval/reports/<ISO>-<surface>.json (gitignored). Path A shim: bypasses the Next.js HTTP route — constructs the same ChatToolContext + tool registry + system prompt v2 + cost budget that chat-stream-service uses, then calls generateText (non-streaming, since the eval cares about final text + tool-call sequence, not streaming). Requires EVAL_USER_ID + EVAL_TEAM_ID env vars (service-role impersonation; no auth token / password needed). PRD-033 Part 3 cutover retired the legacy surface, so `--surface=old` now errors with a deprecation message
...
| `structured_notes` | TEXT | Nullable. Legacy markdown rendering of the extraction output. Since PRD-031 Part 1, **no longer written on session create or re-extraction** — populated only for sessions captured before PRD-031 Part 1, and for user-driven manual markdown edits on legacy rows. The master-signal backend renders markdown on demand from `structured_json`; this column is read directly only as a fallback for pre-PRD-018 rows that have no JSON. Hard-delete deferred to a future PRD. |
| `structured_json` | JSONB | Nullable. Schema-validated JSON extraction output (PRD-018). Single source of truth for extraction since PRD-031 Part 1; markdown is derived on demand by consumers that still need it. PRD-031 Part 2 bumped `EXTRACTION_SCHEMA_VERSION` to `2` (added `positiveSignals` array). New rows tag themselves `schemaVersion: 2`; existing v1 rows are not re-validated and remain readable via defensive `?? []` access on the new field. |
...
| `storage_path` | TEXT | Nullable since PRD-032 Part 2 (migration `001-make-storage-path-nullable.sql`). Non-null path in Storage bucket (`{ownerId}/{sessionId}/{uuid}.{ext}`) for parsed-file rows; NULL for `source_format = 'video_transcript'` rows (no original blob — transcript is the only artefact). |
| `parsed_content` | TEXT | NOT NULL, extracted text from file (or transcript text for video transcripts) |
| `source_format` | TEXT | NOT NULL, detected format (`generic`, `whatsapp`, `slack`, `video_transcript`) |
| `created_by` | UUID | Default `auth.uid()` |
| `team_id` | UUID (FK → teams) | Nullable. NULL = personal workspace. |
| `created_at` | TIMESTAMPTZ | Default `now()` |
| `deleted_at` | TIMESTAMPTZ | Soft delete |
| `last_edited_at` | TIMESTAMPTZ | Nullable (PRD-032 Part 3, migration `002-add-transcript-edit-tracking.sql`). NULL = never edited (default — including all parsed-file rows, which are not editable). Non-null = transcript was edited (either pre-save by the user with `is_edited=true` on the upload payload, or via PATCH after persistence). The "edited" badge in the UI is shown when this column is non-null; the tooltip shows `formatRelativeTime(last_edited_at)`. Application-layer rule: editing is exclusive to `source_format = 'video_transcript'`. |
...
Persistent workspace-level notifications. **One row per recipient** (per PRD-029 Part 7's per-user fan-out reshape — broadcasts are exploded at emit time into per-recipient rows; a row's `user_id` is its sole audience). Event-type strings are governed by the closed application-layer registry in `lib/notifications/events.ts` — there is intentionally no DB-level CHECK constraint, so adding a new event type is a single-file code change.

| Column | Type | Notes |
|--------|------|-------|
| `id` | UUID (PK) | `gen_random_uuid()` |
| `team_id` | UUID (FK → teams) | **Nullable** (Part 7). NULL = personal-workspace notification. The bell's per-row workspace label uses this; a NULL row renders no label. |
| `user_id` | UUID (FK → auth.users) | **NOT NULL** (Part 7). The recipient. Every row has exactly one audience member; broadcasts are fanned out at emit time. |
...
| `read_at` | TIMESTAMPTZ | Nullable. Set by `markRead` / `markAllRead`; idempotent — re-reads are no-ops. Per-user since Part 7 (the row is per-recipient). |
| `expires_at` | TIMESTAMPTZ | Nullable. Per-row override for retention; deleted by Part 5's cleanup before the default age cutoff fires. Filtered out of bell read paths since Part 6 (`expires_at IS NULL OR expires_at > now()`). |
| `created_at` | TIMESTAMPTZ | NOT NULL, default `now()`. |

**Indexes (post-Part 7):** `(user_id, created_at DESC)` for the dropdown listing; `(user_id) WHERE read_at IS NULL` for the unread badge; `(expires_at) WHERE expires_at IS NOT NULL` for Part 5's cleanup pass. The `(team_id, created_at DESC) WHERE user_id IS NULL` partial index from Part 1 was dropped in Part 7's migration — broadcasts no longer exist as a row shape.
**RLS (post-Part 7):** SELECT and UPDATE policies grant access to rows where `user_id = auth.uid()`. Cross-user leakage is structurally impossible — each row has exactly one recipient, the predicate matches that one recipient, and there are no other code paths into the table from the authenticated role. INSERT and DELETE have no policies, so service-role-only writes (the `notification-service.emit` fan-out path and Part 5's `deleteExpired` path). Column-scope on UPDATE (only `read_at` mutates) is enforced at the service layer, not by RLS.
...
**Service emit fan-out (Part 7):** `notification-service.emit({ eventType, teamId?, userId?, actorId?, payload })` resolves a recipient set: targeted emit (`userId` set) returns `[userId]`; broadcast emit (`teamId` set, no `userId`) returns active members of the team via `repo.listActiveTeamMemberIds(teamId)`, optionally minus `actorId` (P7.R3 actor suppression). The recipient set is bulk-inserted via `repo.bulkInsert(...)`. Targeted emits ignore `actorId` because the named user is the audience by design.
...
| POST | `/api/files/transcribe` | **2026-05-13 (chunked transcribe):** now stateless and per-chunk only — the auto-persist branch was lifted out into `/api/files/transcribe/finalize` so the chunked client pipeline can persist once after stitching, not once per chunk. Receives one MP3 segment + original-video metadata, transcribes via OpenAI Whisper (`experimental_transcribe`) behind `resolveTranscriptionModel()`. Audio is held in memory only; never persisted. Wire format: `multipart/form-data` with `audio` Blob + `video_file_name` + `video_file_type` (in `VIDEO_MIME_TYPES`) + `video_file_size` (≤ 500 MB) + `duration_seconds` (≤ 7200). No `session_id` accepted. Audio capped at 25 MB → 413 (Whisper's hard limit; each chunk is ~2.16 MB by design). Returns `{ parsed_content, file_name, file_type, file_size, duration_seconds, source_format: "video_transcript" }`. Errors: 400 (validation), 413 (audio over limit), 422 (empty transcript per P2.R8), 500 (config error), 502 (upstream Whisper failure after retry exhaustion). `maxDuration = 60` (Hobby cap, explicit). | Yes |
| POST | `/api/files/transcribe/finalize` | **2026-05-13 (chunked transcribe):** persist-only — no Whisper call. Called by the chunked client pipeline after all per-chunk POSTs return and the client has stitched the transcripts. Body: JSON `{ session_id (uuid), parsed_content, video_file_name, video_file_type, video_file_size, duration_seconds, is_edited? }`. Validates session access via `requireSessionAccess`, enforces `MAX_ATTACHMENTS` and `MAX_COMBINED_CHARS`, calls `createTranscriptAttachment` + `sessionRepo.markStale`. Returns `{ parsed_content, file_name, file_type, file_size, duration_seconds, source_format, attachment }`. Errors: 400 (validation / max attachments / invalid JSON), 422 (combined-char over limit), 500 (persist failure). `maxDuration = 60`. | Yes |
...
| PATCH | `/api/sessions/[id]/attachments/[attachmentId]` | PRD-032 Part 3 — edit a video transcript's `parsed_content`. Body: `application/json` with `{ parsed_content: string }`. Auth via `requireSessionAccess`. Editing is exclusive to `source_format = "video_transcript"` rows (uniform 404 for non-transcripts — doesn't leak whether the row exists vs. isn't editable). Server stamps `last_edited_at = now()` and marks the session stale. Returns `{ attachment }`. Errors: 400 (`parsed_content` missing OR empty per P4.R6 with verbatim message `"Transcript can't be empty. Use Remove if you want to discard this attachment."`), 404 (not found / not a transcript), 422 (combined input over `MAX_COMBINED_CHARS`), 500 (DB failure). | Yes |
...
**Chat tool surface** (PRD-033 Parts 1, 2, 3 — fully wired into the streaming surface as of Part 3 cutover): the agentic primitive tools live in `lib/services/chat-tools/` (one file per tool, factory + Vercel AI SDK `tool()`). The registry `createChatTools(ctx)` returns 11 tools: `list_clients`, `list_sessions`, `list_themes` (discovery), `semantic_search` (hybrid vector+FTS), `fetch_session_content`, `fetch_signals`, `aggregate`, `time_series` (delegates to existing `database-query` domains), `summarise_sessions` (map-reduce against a separately-configured cheap model via `SUMMARY_AI_*` env vars), `insights_latest`, `insights_history`. Workspace scope is enforced at the service layer; tool inputs never carry `teamId`. Production-readiness pieces from Part 3: prompt caching via `chat-prompt-cache.ts` (Anthropic explicit, OpenAI automatic, Google deferred), per-turn cost circuit breaker via `chat-cost-budget.ts` (default 100k tool-result tokens, env-tunable), system prompt v2 in `chat-prompt.ts` (`CHAT_PROMPT_VERSION = "v2"`).
...
3. **Server-side AI calls only, provider-agnostic.** API keys never touch the browser. All AI operations go through Next.js API routes. The AI service exposes two public generics — `callModelText()` and `callModelObject<T>()` — that wrap provider resolution, retry logic, and error classification. All non-streaming LLM calls (extraction, synthesis, title generation, theme assignment) use these generics. Streaming chat uses `streamText()` directly. Supports multiple providers (Anthropic, OpenAI, Google) via `AI_PROVIDER` and `AI_MODEL` env vars. **Active providers today: Anthropic and OpenAI.** Google adapter code exists for symmetry (`@ai-sdk/google` is installed; `PROVIDER_MAP` includes the factory), but Google is not in production use yet — provider-specific decisions deferred until then (see "Deferred when Google becomes an active provider" below).

**Deferred when Google becomes an active provider** — three decisions are intentionally left for the PRD that adds Google to production traffic:
- **Prompt-caching integration in `lib/services/chat-prompt-cache.ts`** (PRD-033 Part 3): replace the no-op fallback with the explicit `createCachedContent` API call before generation, including cache lifecycle management (TTL, eviction, per-conversation cache id storage).
- **Cached-token usage telemetry** in the per-turn `chat-complete` log line: verify Google's SDK-normalised `usage` field exposes cached-token counts at the same path Anthropic and OpenAI use, or adapt the read.
- **Per-tool description ordering** in `createChatTools()`: lock the order before Google traffic is enabled, since reordering invalidates cache prefixes per provider.

This note exists so the next implementer doesn't rediscover these costs in production. Until Google is in production traffic, the existing no-op fallback in `chat-prompt-cache.ts` is correct and honest — Google calls show `cache-hit-input: 0` in the per-turn log line.
...
14. **Chat data isolation: anon client + service-role client.** The chat system uses two Supabase clients: the RLS-protected anon client for `aggregate` / `time_series` / `insights_*` tool reads (delegating to `executeQuery` so RLS enforces both team scoping and personal-workspace isolation); and the service-role client for the embedding repository (similarity / FTS RPCs apply `filter_team_id` + `filter_user_id` in SQL) and for the `ChatQueryRepository` (cross-table joins for `list_*` / `fetch_session_content` / signal-level filter resolution; workspace scope enforced in the repo). `teamId` is always resolved server-side from the `active_team_id` cookie — never accepted from the client request body. **PRD-033 Part 3 cutover** replaced the two-tool surface (`searchInsights` + `queryDatabase` with a 15-action enum) with the 11-tool primitive surface in `lib/services/chat-tools/`; per-tool filter contracts replaced the shared 8-field filter bag, and the defensive cue-matching sanitisation layer was deleted because per-tool Zod schemas eliminate schema-fill hallucination at the boundary. Production-readiness pieces: prompt caching (Anthropic explicit, OpenAI automatic, Google deferred), per-turn cost circuit breaker (default 100k tool-result tokens), eval-gated cutover (revert is the rollback mechanism — no feature flag system).
